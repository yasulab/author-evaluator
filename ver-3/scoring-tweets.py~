...
if __name__ == "__main__":
    ...
    if not argv_len == 4:
        print "Usage: python main.py FILENAME_1 FILENAME_2 TEXT"
        exit()
    filename_1 = sys.argv[1]
    filename_2 = sys.argv[2]
    given_text = sys.argv[3]
    
    ptable_1 = create_log_ptable_from_filename(filename_1)
    ptable_2 = create_log_ptable_from_filename(filename_2)

    print "******************"
    print "*%s *" % filename_1
    print "******************"        
    show_all_ptable(ptable_2)   
    show_all_ptable(ptable_1)
    print "******************"
    print "*%s *" % filename_2
    print "******************"        
    show_all_ptable(ptable_2)

    score_1 = get_score(ptable_1, given_text)
    score_2 = get_score(ptable_2, given_text)
    
    print "Given Text:", given_text
    print "Score for '%s': %d" % (filename_1, score_1)
    print "Score for '%s': %d" % (filename_2, score_2)

    if score_1 == score_2:
        print "Can't determine who is the author."
    elif score_1 > score_2:
        print "Given text's author will be the author of '%s'." % filename_1
    else:
        print "Given text's author will be the author of '%s'." % filename_2
